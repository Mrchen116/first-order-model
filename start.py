...
if not parser.cutted:
    with mp_face_detection.FaceDetection(
        model_selection=1) as face_detection:
        image = cv2.imread(parser.source_image)
...
exit(0)
driving_video = parser.driving_video
source_image = parser.source_image
result = os.path.join('log', f"{getfilename(driving_video)}_{getfilename(source_image)}.mp4") if parser.result_video == '0' else parser.result_video
